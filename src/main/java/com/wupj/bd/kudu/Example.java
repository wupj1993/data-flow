...
public class Example {
    ...
    static void createExampleTable(KuduClient client, String tableName) throws KuduException {
        ...
        if (b) {
            System.out.println("表存在进行删除");
            client.deleteTable(tableName);
        }
        // Set up a simple schema.
        List<ColumnSchema> columns = new ArrayList<>(17);
        // 主键
        columns.add(new ColumnSchema.ColumnSchemaBuilder("PK_SCC_SALEINFO", Type.STRING)
                .key(true)
                .build());
        // 年份
        columns.add(new ColumnSchema.ColumnSchemaBuilder("CYEAR", Type.STRING)
                .build());
        // 月份
        columns.add(new ColumnSchema.ColumnSchemaBuilder("CMONTH", Type.STRING)
                .build());
        // 销售日期
        columns.add(new ColumnSchema.ColumnSchemaBuilder("DSALEDATE", Type.STRING)
                .build());
        // 工业公司编码
        columns.add(new ColumnSchema.ColumnSchemaBuilder("VINSTITUTIONCODE", Type.STRING)
                .build());
        // 销售单位
        columns.add(new ColumnSchema.ColumnSchemaBuilder("CCUSTOMERCODE", Type.STRING)
                .build());
        columns.add(new ColumnSchema.ColumnSchemaBuilder("CINVENTORYCODE", Type.STRING)
                .build());

        columns.add(new ColumnSchema.ColumnSchemaBuilder("VBRAND", Type.STRING)
                .build());
        // 香烟等级
        columns.add(new ColumnSchema.ColumnSchemaBuilder("VINVLEVEL", Type.STRING)
                .build());
        // 销售城市
        columns.add(new ColumnSchema.ColumnSchemaBuilder("VCITYCODE", Type.STRING)
                .build());
        // 销售省份
        columns.add(new ColumnSchema.ColumnSchemaBuilder("VPROVINCECODE", Type.STRING)
                .build());
        // Type.DECIMAL 需要指定typeAttribute
        /*columns.add(new ColumnSchema.ColumnSchemaBuilder("NNUM", Type.DECIMAL)
                .typeAttributes(DecimalUtil.typeAttributes(20,
                        8)).build());
        // 需要指定精度
        columns.add(new ColumnSchema.ColumnSchemaBuilder("NMNY", Type.DECIMAL)
                .typeAttributes(DecimalUtil.typeAttributes(20,
                        8)).build());*/
        columns.add(new ColumnSchema.ColumnSchemaBuilder("NNUM", Type.INT32).build());
        // 需要指定精度
        columns.add(new ColumnSchema.ColumnSchemaBuilder("NMNY", Type.INT32).build());
        columns.add(new ColumnSchema.ColumnSchemaBuilder("TS", Type.STRING).nullable(true)
                .build());
        columns.add(new ColumnSchema.ColumnSchemaBuilder("DR", Type.INT32).nullable(true)
                .build());
        // 时间官网用了INT64 来表示
        columns.add(new ColumnSchema.ColumnSchemaBuilder("CREATE_TS", Type.INT64).nullable(true)
                .build());
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Run with -DkuduMasters=master-0:port,master-1:port,... to override.");
        System.out.println("-----------------------------------------------");
        String tableName = "YYTERP_SCC_SALEINFO";
        KuduClient client = new KuduClient.KuduClientBuilder(KUDU_MASTERS).build();
        try {
            createExampleTable(client, tableName);
